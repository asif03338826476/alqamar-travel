...
export const Navbar = (props: Props) => {
  ...
  return (
    <div
      className={` w-full flex flex-wrap items-center justify-between lg:justify-start py-10 px-10 lg:pl-[10%] transition-all overflow-hidden ${
        nav ? "h-[320px]" : "h-[90px]"
      } `}
    >
      <h1 className="text-2xl font-bold flex flex-col mr-[40px] font-heading">
        <span className="font-regular font-sans text-xs text-center">
          Al-Qamar
        </span>
        Travels
      </h1>
      <button
        className="hover:bg-slate-100 p-2 rounded-[50%] lg:hidden "
        onClick={() => setNav((e) => !e)}
      >
        <MenuIcon />
      </button>
